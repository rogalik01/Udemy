...
const Form = () => {
    ...
    const myRef = useRef(1); // изменение не вызывает ререндеринг

    useEffect(() => {
        // myRef.current++; // для отслеживания кол-ва перерендеринга
        myRef.current = text; // Для сохранения предыдущего стейта
